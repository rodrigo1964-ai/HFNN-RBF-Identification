"""
generate_figures.py - Generate all figures for Case Study 1

Generates high-quality figures for publication:
1. Comparison between regressor and odeint
...
def figure_1_comparison():
    """
    Figure 1: Direct comparison between regressor and odeint.
    """
    ...
    axes[0].set_title("Case 1: $y' + y^2 = \sin(5t)$ with $y(0) = -0.2$", fontweight='bold')
    ...
    plt.tight_layout()
    plt.savefig('case1_comparison_N500.png', dpi=300, bbox_inches='tight')
    print("  ✓ case1_comparison_N500.png")
    plt.close()
...
def figure_2_convergence():
    ...
    ax.set_ylabel('Error')
    ax.set_title("Case 1: Homotopic Regressor Convergence Analysis", fontweight='bold')
    ax.legend(loc='best')
    ...
    plt.tight_layout()
    plt.savefig('case1_convergence.png', dpi=300, bbox_inches='tight')
    print("  ✓ case1_convergence.png")
    plt.close()
...
def figure_3_multiple_n():
    ...
    for idx, n in enumerate(n_values):
        ...
        axes[idx].plot(t_rk4, y_rk4, 'b-', linewidth=2, label='odeint (reference)')
        axes[idx].plot(t_reg, y_reg, 'ro', markersize=2, label='Homotopic Regressor', alpha=0.6)
    ...
    plt.suptitle("Case 1: Effect of Number of Points $N$", fontsize=14, fontweight='bold')
    plt.tight_layout()
    plt.savefig('case1_multiple_N.png', dpi=300, bbox_inches='tight')
    print("  ✓ case1_multiple_N.png")
    plt.close()
...
def figure_4_phase_portrait():
    ...
    ax.plot(y_rk4, dydt_rk4, 'b-', linewidth=2, label='odeint (reference)', alpha=0.7)
    ax.plot(y_reg, dydt_reg, 'r--', linewidth=1.5, label='Homotopic Regressor', alpha=0.8)
    ax.plot(y0, dydt_rk4[0], 'go', markersize=10, label='Initial condition', zorder=5)

    ax.set_xlabel('$y$')
    ax.set_ylabel("$y'$")
    ax.set_title("Case 1: Phase Portrait", fontweight='bold')
    ax.legend(loc='best')
    ax.grid(True, alpha=0.3)

    plt.tight_layout()
    plt.savefig('case1_phase_portrait.png', dpi=300, bbox_inches='tight')
    print("  ✓ case1_phase_portrait.png")
    plt.close()
...
if __name__ == "__main__":
    print("=" * 70)
    print("Generating all figures for Case Study 1")
    print("=" * 70)
    print()
